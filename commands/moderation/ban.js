...
module.exports.run = (client, message, args,) => {
  try {
    message.delete();
    let user = message.mentions.users.first();const error24 = new MessageEmbed()
const error25 = new MessageEmbed()
    ...
    user ? message.guild.member(user).ban(reason) : message.channel.send(error25);
    message.delete();
  ...
  let result = Math.floor((Math.random() * banList.length))

  const embed = new MessageEmbed()
    .setAuthor(`**Ban ${banList[result]}** | ${user.username}`, user.user.displayAvatarURL())
    .setColor("#ffa500")
    .addFields(
    ...
    .setTimestamp()
    .setFooter(`Banni à:`, message.author.displayAvatarURL());
    
  client.channels.cache.get(settings.logChannel).send(embed);
  //client.channels.cache.get('707211544122884131').send(`:warning: Membre Ban: <@${member.id}>`); // A MODIFIER SPECIAL SERVEUR
  } catch (error) {
