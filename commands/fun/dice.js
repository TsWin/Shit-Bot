...
module.exports.run = async (client, message, args) => {   
    try {
        const diceImg = new MessageAttachment("../../assets/emoji/dice.png");
    const randomDice = () => Math.floor(Math.random() * 6) + 1;
    const dicemEbed = new MessageEmbed()
    .setColor("RANDOM")
    .setTitle(`Lancé de dé`)
    .addField(`Résultat:`, `${randomDice()}`)
    //.attachFiles(['../../assets/emoji/dice.png'])
    //.setImage('attachment://dice.png')
    .setFooter(`Demandé par ${message.author.username}`)
    message.channel.send(dicemEbed);
    } catch (error) {
